// ✅ /components/FutureDiscovery.js — Advanced Stock Analysis with Mini Charts & TP/SL
...
export default function FutureDiscovery({ favorites = [], toggleFavorite }) {
  ...
  useEffect(() => {
    ...
    async function loadSymbol(sym) {
      try {
        ...
        setData((prev) => ({
          ...prev,
          [sym]: { symbol: sym, company: COMPANY_MAP[sym], status: "error" }
        }));
      }
    }
  ...
  const list = DEFAULT_SYMBOLS.map((sym) => data[sym]).filter(Boolean);
  ...
  return (
    <section className="w-full bg-[#0b1220] min-h-screen text-gray-100 px-3 pt-3 font-sans pb-24">
      ...
      {isLoading ? (
        <div className="text-center text-gray-400 py-10 italic">⏳ Loading advanced analysis...</div>
      ) : (
